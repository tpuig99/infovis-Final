...
tags_metadata = [
    ...
    {"name": "Vacunas solo por provincia de residencia", "description": "Obtiene las vacunas filtradas por la jurisdicción/provincia de residencia"},
    {"name": "Vacunas solo por marca de vacuna suministrada", "description": "Obtiene las vacunas filtradas por la marca de la vacuna suministrada"},
    {"name": "Vacunas solo por número de dosis", "description": "Obtiene las vacunas filtradas por si es primera o segunda dosis"},
    {"name": "Vacunas solo por provincia donde se aplició la dosis", "description": "Obtiene las vacunas filtradas por la jursidicción/provincia en la que se aplicó la dosis"},
...
def read_vacunas(dosis:models.ModelDosis,db: Session = Depends(get_db)):
    vacunas = crud.get_vacunas_by_dosis_count(db,dosis)
    return vacunas    
...
def read_vacunas(age:models.ModelAge, dosis:models.ModelDosis,  skip: int = 0, limit: Optional[int] = Query(None, description="No es un campo obligatorio, sin embargo por la gran cantidad de datos recomendamos darle uso."), db: Session = Depends(get_db)):
    vacunas = crud.get_vacuna_by_age_dosis(db,age=age,dosis=dosis, limit=limit, skip=skip)
    return vacunas
...
def read_vacunas(age:models.ModelAge, dosis:models.ModelDosis,db: Session = Depends(get_db)):
    vacunas = crud.get_vacuna_by_age_dosis_count(db,age=age,dosis=dosis)
    return vacunas
